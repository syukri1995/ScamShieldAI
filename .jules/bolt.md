...
## 2024-05-18 - Optimize URL detection with pre-compiled regex and sets
**Learning:** For performance-sensitive string matching and extraction operations, using inline `re.findall` or `re.search` causes significant overhead because Python has to perform internal caching dictionary lookups on every single call. In functions called frequently or in loops, this overhead adds up. Furthermore, appending flags to lists and doing `set()` later causes O(N) lookup issues during repeated additions, compared to O(1) in a native set.
**Action:** Always pre-compile regex patterns at the module level (e.g. `URL_PATTERN = re.compile(...)`) when doing repeated text extractions. Use native `set()` directly when collecting distinct boolean flags to prevent O(N) operations in `append()` + `set()` aggregations.
