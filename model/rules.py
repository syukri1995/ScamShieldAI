...
# Pre-compile regexes for performance optimization (avoiding re-compilation in loops)
...
def apply_rules(text: str) -> dict:
    ...
    # Use a native set to efficiently deduplicate flags during validation
    url_flags = set()
    ...
    if url_flags:
        sorted_url_flags = sorted(url_flags)
        reasons.append(f"URL warning signals: {', '.join(sorted_url_flags)}")
